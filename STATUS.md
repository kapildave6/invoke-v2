...
| Fallback commands | ✅ | **user-curated ordered list (Settings → Commands → Fallback Commands); shown in root only on no-match; query as `launchProps.fallbackText`; `disabledByDefault` opt-in toggle (Chunk Fallback, 2026-06-21)** |
...
| `@invoke/utils` hooks | 🟡 | usePromise/useFetch(options)/useCachedState/useCachedPromise/useExec/useSQL/useForm/useLocalStorage/useFrecencySorting/useAI present; **pagination + runtime `mutate` landed 2026-06-21; `optimisticUpdate`/`rollbackOnError` landed (Chunk H, 2026-06-21)**. useCachedPromise cache-write follow-up. **`useStreamJSON` top-level array streaming landed (Chunk I-stream, 2026-06-21)** via `createArrayStreamParser`; load-whole fallback for `dataPath`/non-array/malformed JSON. Residuals: `dataPath` nested-array streaming; progressive mid-stream `setData`. **Pending:** AI token streaming |
...
| AI Extension Tools / MCP | ⬜ | the genuinely-absent extension APIs. **Window Management API DONE (Chunk WM-2, 2026-06-23): AX-backed `getActiveWindow`/`getWindowsOnActiveDesktop`/`setWindowBounds`/`getDesktops` + types — no longer in this row.** _(2026-06-21: `launchCommand`, `updateCommandMetadata`, `BrowserExtension`, the `Keyboard` namespace, and `menu-bar`/`MenuBarExtra` are now implemented — see Appendix A.)_ **Named-type exports: `LaunchProps`, `PreferenceValues`, `Preferences` (augmentable), `Navigation`, `Form.Values`/`FormValues`, `Keyboard.KeyModifier`/`KeyEquivalent` exported (Chunk I2, 2026-06-21); `LaunchContext`, `Form.Event`/`Form.Event.Type`, `Image.ImageLike`/`Image.Source` exported (Chunk I-residuals, 2026-06-21) — named-type export item fully DONE.** Remaining absent: `Cache.*` sub-types (low priority). Host AI key (`ANTHROPIC_API_KEY`) scrubbed from extension child env (Chunk I-residuals, 2026-06-21). |
...
| Window management (maximize/halves/quarters/center/thirds + cycling + custom) | ✅ | AX move/resize; maximize, halves, quarters, center, **First/Center/Last Third + First/Last Two-Thirds**, **½→⅔→⅓ cycling** (WM-1), and **user-defined Custom Window Management Commands + multi-app Window Layout Commands via a Raycast-style two-pane designer** (a dedicated 900×588 modal: left = live desktop preview with each app's window drawn + click-to-select + "+ Add"; right = inspector with 9-anchor Position + Size Auto/pt + Offset pt; Create/Edit/Delete, bindable — WM-3/builder-parity + layout-designer 2026-06-23/25). Root commands + per-command hotkeys (needs Accessibility); **Respect Stage Manager** toggle (best-effort strip inset) + **Apply Window Hotkey Preset** (Arrows / Vim). **Full WM parity COMPLETE 2026-06-25: WM-1 cycling/thirds + WM-2 extension `WindowManagement` API + WM-3 Create Window Command + WM-4 Create Window Layout (two-pane designer) + WM-5 Stage Manager/presets — all DONE.** |
...
- **P1 — depth:** _(Done 2026-06-21: Form PasswordField (masked) + DatePicker (native); Detail.Metadata clickable Link + colored TagList chips + colored Label/icon; List/Grid pagination; grouped `ActionPanel.Section` + drill-in `Submenu`; `List`/`Detail` `isLoading`; List/Grid accessories incl. `Color`/`Icon` tint + `FileIcon`; `open`/`trash`/`showInFinder`; `mutate` runtime. controlled searchText/throttle/filtering + Dropdown storeValue landed (Chunk F, 2026-06-21). **`Alert.Options` `icon`/`dismissAction.style`/`rememberUserChoice` + `Clipboard.read` full `{text,html,file}` + `Clipboard.clear` landed (Chunk G, 2026-06-21). `Toast` primary/secondary actions (`Toast.ActionOptions`) landed (Chunk G′, 2026-06-21) — P1 feedback group (Toast actions + Alert.Options + Clipboard) now complete. `TagList.Item` `onAction` + TagList overflow-wrap + `DatePicker` `min`/`max`/type + typed `Date` onChange + `optimisticUpdate`/`rollbackOnError` landed (Chunk H, 2026-06-21).**)_ **`Clipboard.read` `offset` (Nth clipboard-history entry) landed (Chunk Clipboard-offset, 2026-06-22)** — last cleanly-tractable micro-residual. Remaining (deferred, NOT micro-fixes per the 2026-06-22 investigation): `isFullDay()` (needs picker *type* threaded through the value); TagPicker→array (needs a real multi-select control); useCachedPromise optimistic cache-write.
- **P2 — breadth / v2:** _(Done 2026-06-21: `menu-bar` + `NSStatusItem` + `MenuBarExtra.*`; `launchCommand`; `updateCommandMetadata`; `BrowserExtension`; `interval` for no-view. **`Icon` coverage + `Color.Dynamic` + `Image.Mask` + dynamic light/dark images landed (Chunk I, 2026-06-21):** 102-member `Icon` enum + `IconSymbol` validated fallback; `Color.Dynamic({light,dark})` exported; `Image.Mask` Circle/RoundedRectangle honored; dynamic `{source:{light,dark}}` dispatched by appearance. Residual tail: literal-complete ~250-member `Icon` enum; `Image.tintColor` top-level export; appearance-change live re-render. **Named-type exports + real `environment` fields (Chunk I2, 2026-06-21):** `LaunchProps`, `PreferenceValues`, `Preferences` (augmentable), `Navigation`, `Form.Values`/`FormValues`, `Keyboard.KeyModifier` (incl. "win"), `Keyboard.KeyEquivalent` exported; `environment.appearance` real (NSApp.effectiveAppearance, main-thread-guarded); `textSize` host-provided; `canAccess(AI)` real (AIService.hasStoredKey()). Still pending: live appearance-change push; non-AI `canAccess`. **`LaunchContext`/`Form.Event`/`Form.Event.Type`/`Image.ImageLike`/`Image.Source` exported (Chunk I-residuals, 2026-06-21)** — named-type export item fully DONE. **`MenuBarExtra.Item` `alternate`/`shortcut` + click `ActionEvent` (`{type:"left-click"}`) + `MenuBarExtra.ActionEvent` type + typed `MenuBarItemProps` DONE (Chunk I-menubar, 2026-06-21)** — caveats: right-click distinction not possible (NSMenuItem); named-key `KeyEquivalent` mapping latent.)_ AI streaming + Tools/MCP/Skills; OAuth provider presets; export remaining named types (`Cache.*` sub-types only). **Window Management: WM-1 (cycling/thirds) + WM-2 (extension `WindowManagement` API, AX-backed) + WM-3/builder-parity (Create Window Command — Raycast-style 9-anchor + Size(Auto/pt) + Offset + live-preview editor, bindable customs) + WM-4 Create Window Layout (two-pane designer: live preview + inspector) + WM-5 (Respect Stage Manager toggle + Apply Window Hotkey Preset) DONE 2026-06-23/25 — full Window-Management parity COMPLETE.** _(Fallback commands + `disabledByDefault` DONE — Chunk Fallback, 2026-06-21.)_ _(Note: `useStreamJSON` top-level streaming DONE — Chunk I-stream, 2026-06-21; residuals: `dataPath` nested-array streaming + progressive mid-stream `setData`.)_
