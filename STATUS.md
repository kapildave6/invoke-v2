...
- **P1 — depth:** controlled `searchText`/`throttle`; per-tag `TagList.Item` `onAction` + TagList wrapping; `DatePicker` `min`/`max`; typed field values; `Toast`/`Alert.Options` actions; `optimisticUpdate`/`rollbackOnError`; full `Clipboard.read`+`clear`. _(Done 2026-06-21: Form PasswordField (masked) + DatePicker (native); Detail.Metadata clickable Link + colored TagList chips + colored Label/icon; List/Grid pagination; grouped `ActionPanel.Section` + drill-in `Submenu`; `List`/`Detail` `isLoading`; List/Grid accessories incl. `Color`/`Icon` tint + `FileIcon`; `open`/`trash`/`showInFinder`; `mutate` runtime.)_
