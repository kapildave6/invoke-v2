...
- **P1 — depth:** List/Grid pagination + controlled `searchText`/`throttle`; clickable `Detail.Metadata.Link` + per-tag `TagList.Item`; `PasswordField` masking + native `DatePicker`; typed field values; `Toast`/`Alert.Options` actions; `optimisticUpdate`/`rollbackOnError`; full `Clipboard.read`+`clear`. _(Done 2026-06-21: grouped `ActionPanel.Section` + drill-in `Submenu`; `List`/`Detail` `isLoading`; List/Grid accessories incl. `Color`/`Icon` tint + `FileIcon`; `open`/`trash`/`showInFinder`; `mutate` runtime.)_
