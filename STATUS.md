...
| Window management (maximize/halves/quarters/center/thirds + cycling + custom) | ✅ | AX move/resize; maximize, halves, quarters, center, **First/Center/Last Third + First/Last Two-Thirds**, **½→⅔→⅓ cycling** (WM-1), and **user-defined Custom Window Management Commands + multi-app Window Layout Commands via a Raycast-style two-pane designer** (a dedicated 900×588 modal: left = live desktop preview with each app's window drawn + click-to-select + "+ Add"; right = inspector with 9-anchor Position + Size Auto/pt + Offset pt; Create/Edit/Delete, bindable — WM-3/builder-parity + layout-designer 2026-06-23/25). Root commands + per-command hotkeys (needs Accessibility). Full WM parity: **WM-1 cycling/thirds + WM-2 extension API + WM-3 Create Window Command + WM-4 Create Window Layout (Raycast 9-anchor editor) DONE; only WM-5 Stage Manager/presets remains** |
