...
| Window management (maximize/halves/quarters/center/thirds + cycling + custom) | ✅ | AX move/resize; maximize, halves, quarters, center, **First/Center/Last Third + First/Last Two-Thirds**, **½→⅔→⅓ cycling** (WM-1), and **user-defined Custom Window Management Commands via a visual 12×8 grid picker** (Create/Edit/Delete, bindable — Chunk WM-3 2026-06-23). Root commands + per-command hotkeys (needs Accessibility). Full WM parity in progress: **WM-1 cycling/thirds + WM-2 extension API + WM-3 custom commands DONE; WM-4 layouts + WM-5 Stage Manager/presets remain** |
