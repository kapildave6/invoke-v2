...
| Window Management API — `getActiveWindow` / `getWindowsOnActiveDesktop` / `getDesktops` / `setWindowBounds` + `WindowManagement.Window` / `.Desktop` / `.DesktopType` | ✅ | AX-backed (Chunk WM-2, 2026-06-23): `WindowEnumerator` (InvokeServices) enumerates windows via AX, stable ids via `_AXUIElementGetWindow` (dlsym, UUID fallback) + an id→element cache for `setWindowBounds`; AX-native coords. `getDesktops` modeled honestly as one desktop per `NSScreen` (macOS Spaces aren't publicly enumerable — not faked). Needs Accessibility (rejects with a clear message otherwise) |
...
- ~~`Form.DatePicker` `min`/`max` + `isFullDay()`; typed (non-string) field values~~ — **DONE for min/max + date/datetime type + DatePicker→Date (Chunk H, 2026-06-21)**; `isFullDay()` + TagPicker→array **deferred — not micro-fixes** (verified 2026-06-22): `isFullDay()` needs the picker *type* threaded through the value (the host emits a full ISO timestamp even for a date-only picker, so a midnight-heuristic would be wrong); TagPicker→array needs a real multi-select control (today degraded to a single-select dropdown — array-wrapping that would be a janky half-measure). Both surfaced as real-feature items.
...
- ~~`Clipboard.read` `offset` (Nth history entry)~~ — **DONE (Chunk Clipboard-offset, 2026-06-22)**: reads the Nth-back entry from `ClipboardHistory` (offset 0/absent = live pasteboard; concealed clips already excluded, so password-manager copies never appear). _(full `{text,html,file}` read + `Clipboard.clear` DONE 2026-06-21, Chunk G)_
...
- ~~Window Management API (`getActiveWindow` / `getDesktops` / `setWindowBounds` + types)~~ — **DONE (Chunk WM-2, 2026-06-23)**: AX-backed via `WindowEnumerator`; `getDesktops` = one desktop per display (Spaces not faked); needs Accessibility
